...
export class Odm {
    ...
    public static applyODM(odm: ODM, filter: any) {
        ...
        if (odm && filter) {
            Object.keys(filter).forEach((key) => {
                if (odm.fields[key] && odm.fields[key].fieldDetails && odm.fields[key].fieldDetails.type === ElementType.identifier && filter[key]._bsontype !== 'ObjectID') {
    ...
    public static transform<T extends ArrTransform<T>[] | object>(metadata: ODM, value: T, transformDirection?: TransformDirection): T {
        if (!value)
            return;
        let result;
    ...
    private static transofmer(element, metadata: ODM, transformDirection?: TransformDirection) {
        return Object.keys(element).reduce((previousValue: {}, currentValue: string, currentIndex: number, array: string[]) => {
            ...
            if (transformDirection === TransformDirection.IN) {
                ...
                let transformOutMap = metadata.fields[keyElementMeta] || new MetadataField();
            ...
            //TransformDirection.OUT
                ...
                let transformOutMap = metadata.fields[currentValue] || new MetadataField();
                //transform bson type to normal string
                if (transformOutMap.fieldDetails && transformOutMap.fieldDetails.param === 'objectid') {
                    element[transformOutMap.propertyKey] = element[transformOutMap.propertyKey] ? element[transformOutMap.propertyKey].toString() : element[transformOutMap.propertyKey];
                }
    ...
    public static applyObjectID(value: string) {
        try {
    ...
    public static parseToNumber(value: any) {
        try {
            if (isNaN(value)) {
