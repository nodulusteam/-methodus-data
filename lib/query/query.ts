...
export class QueryFragmentElement {
    ...
    and(filter: any | QueryFragmentElement) {
        let newFrag = filter;
        if (filter._isFragment) {
            newFrag = filter.filter;
        }
        if (filter.expression) {
            if (this.expression[Operator.$AND]) {
    ...
    or(filter: any | QueryFragmentElement) {
        ...
        if (filter.tree) {
            if (this.expression[Operator.$OR]) {
...
export class Query {
    ...
    public allowDiskUse: boolean = true;
    ...
    */
            ...
            Object.keys(this.odm.fields).forEach((odmKey) => {
                ...
                if (fieldDetails.type === ElementType.weight) {
                    ...
                    this.queryHelper.arrayActions.forEach((currentArrayActionItem, currentArrayActionItemIndex) => {
                        ...
                        currentActionItemKey = currentActionItemKey;
    ...
    public unwind(_path: string, _includeArrayIndex?: string, _preserveNullAndEmptyArrays?: boolean): Query {
        _preserveNullAndEmptyArrays = _preserveNullAndEmptyArrays || true;
    ...
    public notExists(...args: any[]): Query {
    ...
    public or(filterHolder: any | QueryFragmentElement): Query {
        let match = this.queryHelper.findOperator(Operator.$MATCH);
    ...
    public and(filterHolder: any | QueryFragmentElement): Query {
        let match = this.queryHelper.findOperator(Operator.$MATCH);
    ...
    public notIn(fieldName: string, options: Array<any>): Query {
        let match = this.queryHelper.findOperator(Operator.$MATCH);
    ...
    public in(fieldName: string, options: Array<any>): Query {
        let match = this.queryHelper.findOperator(Operator.$MATCH);
        this.queryHelper.buildOrAnd(match, null, {
            [fieldName]: {
                [Operator.$IN]: options
    ...
    public between(field: string, minVal: any, maxVal: any) {
        this.filter({ [field]: { $gte: minVal, $lte: maxVal } });
        return this;
    }


    public async run(returnType: ReturnType = ReturnType.Multi) {
        ...
        try {
            ...
            let queryString = objStringify(actualQuery, { inline: true, indent: 0, singleQuotes: false });
            let baseString = `db.getCollection('${this.odm.collectionName}').aggregate(${queryString})`;
        ...
        try {
            let result = await this.dbConnection.collection(this.odm.collectionName).aggregate(actualQuery, this.getAggregationOptions((this.allowDiskUse ? { 'allowDiskUse': true } : {}))).toArray();
            ...
            if (result && result.length > 0) {
                ...
                if (this.isPaging) {
                    return { results: [], total: 0 }//default response for paging we need to keep the response the same
                }
            ...
            //lets output the query text:
            let queryString = objStringify(this.toQuery(), { inline: true, indent: 0, singleQuotes: false });
            let baseString = `db.getCollection('${this.odm.collectionName}').aggregate(${queryString})`;
            console.error(error, baseString);
            throw (error);
        }
